...
function onEntryClick(e: JQueryEventObject) {
    var entryCell: Element = e.target;
    var entry = entryCell.textContent.trim();

    if (entry == "") return;

    var isLeft: boolean = $(entryCell).hasClass("leftEntry");
...
function onSubmitClick(e: JQueryEventObject) {
...
function onDataReturn(data: any, textStatus: string, jqXHR: JQueryXHR) {
...
function onClearClick(e: JQueryEventObject) {
    $("#leftInput").val("");
    $("#rightInput").val("");

}

function onDataError(jqXHR: JQueryXHR, textStatus: string, errorThrown: string): any {
